...
class ConnectionMetrics:
    """Класс для сбора метрик подключений"""
    ...
    def get_metrics(self) -> Dict[str, int]:
        """Возвращает текущие метрики и сбрасывает счетчики"""
        with self.lock:
...
class ServerLogger:
    ...
    _registry: Dict[str, 'ServerLogger'] = {}  # Реестр созданных логгеров

    @classmethod
    def get_logger(cls, log_file: str) -> Optional['ServerLogger']:
        """
    ...
    def __init__(self, log_file: str, level: str = 'DEBUG', fmt: str = DEFAULT_FORMAT,
        ...
        # Создаем уникальное имя логгера
        self.logger_name = f"file_logger_{hash(log_file)}"

        try:
            self.logger = self._configure_logger()
            # Добавляем логгер в реестр
    ...
    def _configure_logger(self) -> logging.Logger:
        ...
        # Используем уникальное имя логгера
    ...
    def _configure_fallback(self):
        ...
        if not ServerLogger._global_fallback_configured:
            logging.basicConfig(
                level=self.level,
                format=self.format
            )
            ServerLogger._global_fallback_configured = True
    ...
    def reconfigure(self, level: Optional[str] = None, fmt: Optional[str] = None,
                    ...
                    count: Optional[int] = None) -> 'ServerLogger':
        """
    ...
    def start_metrics_reporter(self, interval: int) -> threading.Thread:
        """Запуск фонового потока для отчетов по метрикам"""
        def reporter():
            while True:
                time.sleep(interval)
                current_metrics = self.metrics.get_metrics()
                if current_metrics:
                    self.info(f"Server metrics: {current_metrics}")
